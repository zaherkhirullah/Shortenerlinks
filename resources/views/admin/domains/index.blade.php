      ...
      <table class="table table-striped table-hover table-flip-scroll cf">
        <tbody>
          ...
          <tr>
            ...
            <td>
              <a href="{{$domain->url }}" class="h5 text-success" target="_blank">
                <strong>{{$domain->url}}</strong>
              </a>
              <small class="text-muted block">{{$domain->url}}</small>
            </td>
            <td class="v-middle hidden-xs">{{$domain->created_at}}</td>
            <td class="v-middle hidden-xs">

              <div class="pos-rlt">
                <button class="btn btn-copy text-success" data-clipboard-text="{{$domain->url}}"
                 data-toggle="button">
                 <span class="text">
         ...
         <td class="pull-right">
          <a href="#edit-link-{{$domain->id}}" data-toggle="modal"
          class="text-success" >
          <span class="text">
...
<div class="col-md-8 col-md-offset-2">
  <center> 
    <h2 class="text-danger alert alert-info"> You don't have domains</h2>
  </center>
</div>
