...
class LinkController extends Controller
{
  ...
  // NewItemew for create new item in table(for calling in store).
    ...
    {
        FrontController::beforeFilter($event);
    ...
    public function shorten()
    {
        ...
        if (isset($this->request->data['domain_id'])) {
            $domain = $this->request->data['domain_id'];
        }
        ...
        $data['ad_id'] = $this->request->data['ad_id'];
        $link->status = 1;
        $link->hits = 0;
        $link->method = 1;
    ...
    public function view($alias = null)
    {
        ...
        // $ad_id = $link->ad_id;
        // if (!array_key_exists($ad_id, get_allowed_ads())) {
        //     $ad_id = get_option('member_default_advert', 1);
        // }
        if ($link->user_id == 1) {
            $ad_id = get_option('anonymous_default_advert', 1);
        }

        // No Ads
        if ($plan_direct || $ad_id == 0) {
            $this->updateLinkHits($link);
            $this->addNormalStatisticEntry($link, $ad_id, [
                'ci' => 0,
                'cui' => 0,
                'cii' => 0,
        ...
        if ($plan_disable_captcha ||
            ...
            $campaign_item = $this->getCampaignItem($ad_id, $traffic_source, $country);
    ...
    public function go()
    {
        ...
        $content = $this->calcEarnings($this->request->data, $link, $link->ad_id);

        $this->response->body(json_encode($content));
        return $this->response;
    }

    protected function getCampaignItem($ad_id, $traffic_source, $country)
    {
    ...
    protected function calcEarnings($data, $link, $ad_id)
    {
        /**
         * Views reasons
         * 1- Earn
        ...
        if ($data['country'] == 'Others') {
            // Update link hits
            $this->updateLinkHits($link);
            $this->addNormalStatisticEntry($link, $ad_id, $data, get_ip(), 11);
            $content = [
                'status' => 'success',
        ...
        if (!is_array($cookie)) {
            // Update link hits
            $this->updateLinkHits($link);
            $this->addNormalStatisticEntry($link, $ad_id, $data, get_ip(), 2);
            $content = [
                'status' => 'success',
        ...
        if ('anonymous' == $link->user->username) {
            // Update link hits
            $this->updateLinkHits($link);
            $this->addNormalStatisticEntry($link, $ad_id, $data, $cookie['ip'], 3);
            $content = [
                'status' => 'success',
        ...
        if ($cookie['ip'] != get_ip()) {
            // Update link hits
            $this->updateLinkHits($link);
            $this->addNormalStatisticEntry($link, $ad_id, $data, $cookie['ip'], 6);
            $content = [
                'status' => 'success',
        ...
        if (!isset($_COOKIE['ab']) || in_array($_COOKIE['ab'], [0, 1])) {
            // Update link hits
            $this->updateLinkHits($link);
            $this->addNormalStatisticEntry($link, $ad_id, $data, $cookie['ip'], 4);
            $content = [
                'status' => 'success',
        ...
        if (!$campaign_item) {
            // Update link hits
            $this->updateLinkHits($link);
            $this->addNormalStatisticEntry($link, $ad_id, $data, $cookie['ip'], 8);
            $content = [
                'status' => 'success',
        ...
        if ($campaign_item->campaign->default_campaign) {
            // Update link hits
            $this->updateLinkHits($link);
            $this->addNormalStatisticEntry($link, $ad_id, $data, $cookie['ip'], 9);
            $content = [
                'status' => 'success',
        ...
        if ($this->isProxy()) {
            // Update link hits
            $this->updateLinkHits($link);
            $this->addNormalStatisticEntry($link, $ad_id, $data, get_ip(), 5);
            $content = [
                'status' => 'success',
        ...
        if ($statistics >= get_option('paid_views_day', 1)) {
            // Update link hits
            $this->updateLinkHits($link);
            $this->addNormalStatisticEntry($link, $ad_id, $data, $cookie['ip'], 7);
            $content = [
                'status' => 'success',
        ...
        $statistic->ad_id = $campaign_item['campaign']['ad_id'];
    ...
    protected function addNormalStatisticEntry($link, $ad_id, $data, $ip, $reason = 0)
    {
        if (!$ip) {
            $ip = get_ip();
        }
